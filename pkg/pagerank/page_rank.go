...
// NewPageRank initializes a PageRank calculator with default parameters.
...
// CalcPageRank runs the rank iteration until tolerance is reached.
...
// RemoveSelfLoops deletes self-loop edges before rank computation.
...
func (pr *PageRank) String() string {
	...
	// order
	pr.OrderResults()
	order := pr.GetMaxToMinOrder()
	for _, nId := range order {
		rank := fmt.Sprintf("%f", pr.Nodes[nId].Rank)
		res += string(pr.Nodes[nId].Id) + "\t\t\t" + rank + "\n"
	}
	res += "----------------------------------\n"
	return res
}
...
// Less implements sort.Interface for ascending rank order.
...
// OrderResults sorts internal ranking indexes.
func (pr *PageRank) OrderResults() {
	sort.Sort(pr)
}

// GetMinToMaxOrder returns node IDs sorted by ascending rank.
func (pr *PageRank) GetMinToMaxOrder() []NodeID {
	return pr.sortIndex
}

// GetMaxToMinOrder returns node IDs sorted by descending rank.
